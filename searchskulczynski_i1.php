...
$result=mysqli_query($koneksi,$query);	 
	...
	// tampilkan kotak pencarian dan jumlah hasil pencarian
	  ...
	  echo "<br /><br><table><tr><td></td><td><form action='searchskulczynski_i1.php' method='GET'><br><input type='text' onclick=value='' size='50' name='keyword' value='$keyword' style='width: 500px; height: 30px; font-size: 16px;'>  <input type='submit' value='Cari'></form></td></tr></table>";
...
$query="SELECT g.id id,kata, A, jmlkatakunci, B, jmlkatadok, jmlkatadok-A C, A/(B+(jmlkatadok-A)) kulczynski_i FROM (SELECT *,jmlkatakunci-A B FROM (SELECT y.id,y.kata,y.freq A FROM (SELECT * FROM katakunci WHERE id='0') AS x JOIN (SELECT * FROM token WHERE id!='0' ORDER BY id+0) as Y ON x.kata=y.kata) AS d JOIN (SELECT COUNT(kata) jmlkatakunci FROM katakunci) AS e) AS f JOIN (SELECT id,COUNT(kata) jmlkatadok FROM token GROUP BY id) AS g ON f.id=g.id GROUP BY g.id ORDER BY kulczynski_i DESC";
